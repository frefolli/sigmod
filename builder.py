...
def get_all_files(dir: str) -> Generator[str, None, None]:
  for entry in os.listdir(dir):
    path = os.path.join(dir, entry)
    if os.path.isfile(path):
      yield path
    elif os.path.isdir(path):
      for _ in get_all_files(path):
        yield _

def filter_by_extension(files: Iterable[str], ext: str|list[str]) -> Generator[str, None, None]:
...
def extract_dependencies(path: str) -> Generator[str, None, None]:
...
class SourceGraph:
  ...
  def load_files(self, include_dirs: list[str], source_dirs: list[str]):
    ...
    for dir in include_dirs:
      self.includes += list(filter_by_extension(get_all_files(dir), [".hh", ".hpp", ""]))
    for dir in source_dirs:
      self.sources += list(filter_by_extension(get_all_files(dir), [".cc", ".cpp"]))
  ...
  def compute_dependencies(self):
    ...
    for path in self.includes:
      ...
      for dep in extract_dependencies(path):
