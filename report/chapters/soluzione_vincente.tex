...
Per poter ridurre i dati di dimensionalit\`a quello che si fa \`e identificare un $\alpha$ che sia pari al valore pi\`u grande all'interno dei vettori (indipendentemente da quale dimensione afferisca) e utilizzarlo per effettuare una sorta di normalizzazione:
ogni vettore viene diviso per questo valore $\alpha$, quindi compresso nell'intervallo $[-1,1]$ (i valori eccedenti gli estremi vengono riassegnati all'estremo che superano, ex: $-5 \rightarrow 5$) e infine scalato moltiplicandolo per il valore $127$.
...
Vice versa nella quantizzazione asimmetrica (implementata ma non utilizzata dagli autori) i valori sarebbero compressi nell'intervallo $[0, 255]$.
