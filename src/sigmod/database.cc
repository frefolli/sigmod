...
void Kmeans(const Database& database) {
    const uint32_t ITERATIONS = 1;
    actual_vector_size = 10;
    const uint32_t n_of_clusters = 256;
    ...
    #pragma omp parallel
    {
        ...
        for (uint32_t i = 0; i < n_of_clusters; i++) {
            beholds[i] = i;
            Record& record = database.records[0];
            #pragma omp critical
            {
                for (uint32_t j = 0; j < actual_vector_size; j++) {
                    centroids[i].fields[j] += record.fields[j];
                    centroids[i].fields[j] /= database.length;
                }
            }
        }
